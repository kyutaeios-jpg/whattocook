...
app.post("/api/ingredients/suggest-synonyms", async (req, res) => {
  try {
    ...
    const basePrompt = `아래 재료 목록에서 같은 재료인데 이름만 다른 것들을 적극적으로 찾아줘.

판단 기준 딱 하나: "마트에서 같은 매대에서 같은 상품을 집는가?"
- 같은 상품을 집는다 → 묶기
- 다른 상품을 집는다 → 묶지 않기

기존 등록된 동의어 (이 스타일과 수준을 참고해):
${synExamples}

묶는 예시:
- 표기법/공백: "간 마늘"="다진 마늘", "달걀"="계란", "돈가스소스"="돈가스 소스"
- 수식어/크기: "돼지고기 뒷다리살 덩어리"→"돼지고기 뒷다리살", "주먹보다 작은 감자"→"감자"
- 괄호/용도: "(순한) 라면"→"라면", "찌개용 돼지고기"→"돼지고기", "불고기용 소고기"→"소고기"
- 냉동/건조: "냉동 블루베리"→"블루베리", "건표고버섯"→"표고버섯"
- 통칭: 진간장→"간장", 양조간장→"간장", 황설탕→"설탕", 꽃소금→"소금"
- 색상: "노란 파프리카"→"파프리카"
- 삶은/불린: "불린 당면"→"당면", "삶은 고구마"→"고구마"

묶으면 안 되는 예시 (다른 매대, 다른 상품):
- 국간장≠간장, 참기름≠들기름, 고춧가루≠고추장
- 삼겹살≠대패삼겹살≠목살 (다른 부위)
- 통마늘≠다진 마늘, 구운 김≠김가루 (다른 매대)
- 감자≠감자전분, 계란≠계란후라이 (다른 상품)
- 참치≠참치캔, 까나리액젓≠멸치액젓
- 진라면≠사골라면≠라면 (다른 라면 브랜드)
- 뉴슈가≠설탕, "알"≠"계란" (알은 생선 부위), "고니"≠"닭고기" (고니는 생선 부위)

대표명: 마트에서 검색하면 나오는 가장 일반적이고 짧은 이름.

적극적으로 많이 찾되, 위 기준을 벗어나지 마.
